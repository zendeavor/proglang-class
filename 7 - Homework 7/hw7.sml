...
(* helper functions for comparing real numbers since rounding means we should
...
(* helper function to return the Line or VerticalLine containing points (x1,y1)
 * and (x2,y2). Actually used only when intersecting line segments, but might be
 * generally useful *)
...
(* helper function for interpreter: return value that is the intersection of the
 * arguments: 25 cases because there are 5 kinds of values, but many cases can be
 * combined, especially because intersection is commutative.  Do *not* call this
 * function with non-values (e.g., shifts or lets) *)
fun intersect (v1,v2) =
    case (v1,v2) of
          (NoPoints, _) => NoPoints (* 5 cases *)
        | (_, NoPoints) => NoPoints (* 4 additional cases *)
        | (Point p1, Point p2) =>
            if real_close_point p1 p2 then v1 else NoPoints
        | (Point (x,y), Line (m,b)) =>
            if real_close(y, m * x + b) then v1 else NoPoints
        | (Point (x1,_), VerticalLine x2) =>
            if real_close(x1,x2) then v1 else NoPoints
        | (Point _, LineSegment seg) => intersect(v2,v1)
        | (Line _, Point _) => intersect(v2,v1)
        | (Line (m1,b1), Line (m2,b2)) =>
            if real_close(m1,m2) then
                (if real_close(b1,b2) then
                  v1 (* same line *)
                else
                  NoPoints) (* parallel lines do not intersect *)
            else
        ...
        | (LineSegment seg, _) =>
            (* the hard case, actually 4 cases because v2 could be a point, line,
             * vertical line, or line segment.
             * First compute the intersection of (1) the line containing the
             * segment and (2) v2. Then use that result to compute what we need.
             *)
            (case intersect(two_points_to_line seg, v2) of
                ...
                | Point(x0,y0) => (* see if the point is within the segment bounds *)
                    ...
                    let
                        ...
                        val (x1,y1,x2,y2) = seg
                    in
                        if inbetween(x0,x1,x2) andalso inbetween(y0,y1,y2) then
                            Point(x0,y0)
                        else
                            NoPoints
                    end
                ...
                | LineSegment seg2 =>
                    (* the hard case in the hard case: seg and seg2 are on the
                     * same line (or vertical line), but they could be (1)
                     * disjoint or (2) overlapping or (3) one inside the other or
                     * (4) just touching. And we treat vertical segments
                     * differently, so there are 4*2 cases. *)
                    let
                    ...
                    in
                        if real_close(x1start,x1end) then
                             ...
                             * let segment a start at or below start of segment
                             * b *)
                            let
                            ...
                            in
                                if real_close(aYend,bYstart) then
                                    Point (aXend,aYend) (* just touching *)
                                else if aYend < bYstart then
                                    NoPoints (* disjoint *)
                                else if aYend > bYend then
                                    (* b inside a *)
                                    LineSegment(bXstart,bYstart,bXend,bYend)
                                else
                                    (* overlapping *)
                            ...
                            in
                                if real_close(aXend,bXstart) then
                                    Point (aXend,aYend) (* just touching *)
                                else if aXend < bXstart then
                                    NoPoints (* disjoint *)
                                else if aXend > bXend then
                                    (* b inside a *)
                                    LineSegment(bXstart,bYstart,bXend,bYend)
                                else
                                    (* overlapping *)
 ...
 * for simplicity we have the top-level function take an environment, (which
 * should be [] for the whole program
 * we assume the expression e has already been "preprocessed" as described in
